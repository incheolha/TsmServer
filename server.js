...
function handleDisconnect() {
  ...
  conn.on('error', function(err) {
    ...
    console.log('db서버가 정상적으로 연결디었습니다.... 감사해요')
  });
}

handleDisconnect();

// 여기까지가 mySql Server 연결 기능 
...
var io = require('socket.io').listen(9004,{pingTimeout: 1000, pingInterval: 1000});
io.sockets.setMaxListeners(0);                                                      // Disable memory leak 현상 발생 제거
var apn = require('apn')                                                            // aaple push notification 모듈
var moment = require('moment')                                                      //날짜 정보 format 모듈
...
io.sockets.on('connection', function(clientSocket){
  ...
  var becomeActive = function(sender_id,push,silent,device_token) {
    //이 함수는 무조건 clientSocket.on 보다 아래에 있어야 에러 안뜸. 이유는 모르겠다
    console.log('test: '+sender_id + ' - ' + push + ' - ' + silent + ' - ' + device_token);
    //가입된 유저가 앱을켰거나 처음 회원가입하는 회원일때 invoke되어짐
    var info = new Object()
    info.socket_id = clientSocket.id
    info.user_id = sender_id
    info.isOn = true
    info.push = push
    info.silent = silent
    ...
    //receive messages that have been sent to me when i was offline
    ...
    conn.query(receiveOfflineMessage,[sender_id],function(err,rows){
      if(err){
        ...
        if(rows.length == 0){
          ...
          //console.log('there are offlineMessages on me');
         //var offlineMessages = []
	 //for(var i = 0; i < rows.length; i++){
         //  var info = new Object()
         //   info.c_id = rows[i].c_id
         //   info.sender_id = rows[i].sender_id
         //   info.receiver_id = rows[i].receiver_id
         //   info.message = rows[i].message
         //   info.attachment_url = rows[i].attachment_url
         //   info.time = rows[i].time
         //   info.type = rows[i].type
         //   console.log('there are offlineMessages on me');
	 //   offlineMessages.push(info)
            //io.to(clientSocket.id).emit('newChatMessage',info)
         // }
...
//    clientSocket.removeListener('becomeActive',becomeActive)

  }

  //console.log('listenerCount: '+clientSocket.listenerCount('becomeActive'));
  clientSocket.on('becomeActive',becomeActive)
  ...
  var sendMessage = function(c_id,sender_id,receiver_id,message,attachment_url,time,type){
    ...
    for(var i = 0; i < userList.length; i++){
      if(userList[i].user_id == receiver_id){
        ...
        info.attachment_url = attachment_url
        info.time = time
        info.type = type

        //push notification setup
        if(userList[i].silent == true){//무음
          //note.badge = 1;
          note.contentAvailable = false;
          note.topic = ".dhtprbs";
          note.alert = "New Message"
        }
        if(userList[i].silent == false){
          //note.badge = 1;
        ...
        //userList[i].user_id != receiver_id
        if(i == userList.length - 1){
...
//    clientSocket.removeListener('sendMessage',sendMessage)
  }
...
//    clientSocket.removeListener('background',background)
  }
...
//    clientSocket.removeListener('pushTrue',pushTrue)
  }
...
//    clientSocket.removeListener('pushFalse',pushFalse)
  }
...
//    clientSocket.removeListener('silentTrue',silentTrue)
  }
